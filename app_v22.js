...
const MENUS = {
    ...
    'TPE-SIN': {
        meal_1: [
            ...
            { code: 'MHT1', name: 'IE-HUTONG Beef', chinese: '胡同燒烤牛小排', dessert: 'HT dessert' },
        ],
        // [關鍵設定] M 航線加入前菜選項
        appetizers: [
...
const ROUTES = [
    ...
    { id: 'TPE-OKA', name: 'Taipei - OkA (SS)', type: 'SS' },
    { id: 'TPE-SHI', name: 'Taipei - SHI (SS)', type: 'SS' },
...
const createInitialOrder = (id) => ({
    ...
    // [新增] 預選餐標記
    isPreSelect: false, 
    isAppetizerPreSelect: false, // [新增] 前菜預選標記
    isMeal2PreSelect: false,     // [新增] 第二餐預選標記
    appetizerChoice: '',
    // [NEW] 加入前菜狀態
    appetizerChoice: '', 
    ...
    activeAircraftConfig.rows.forEach(row => {
        ...
        activeAircraftConfig.seatLetters.forEach(letter => {
            ...
            if (currentFlightAircraftName === 'Airbus A330-900neo') {
                ...
                const isEvenRow = rowNum % 2 === 0; // 是否為偶數排 (2, 4, 6, 8)

                if (isEvenRow) {
                    // 偶數排顯示: A, E, F, K
                    if (!['A', 'E', 'F', 'K'].includes(letter)) return;
                } else {
                    // 奇數排顯示: C, D, G, H
                    if (!['C', 'D', 'G', 'H'].includes(letter)) return;
                }
            }
                ...
                if (isFullyStatic) {
                   // Static view (Order Mode or fully completed service)
                    ...
                    if (order.appetizerChoice) {
                        ...
                        itemsContainer.innerHTML += `<div class="text-base mt-1 font-semibold ${textClass} ${appClass}">App: ${order.appetizerChoice}</div>`;
                    }   

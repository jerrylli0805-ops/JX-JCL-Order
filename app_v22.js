...
// 1. 極致壓縮資料 (為了塞進 QR Code)
function compressFlightData() {
    // 只提取「有變動」的座位資料
    const activeOrders = orders.filter(o => 
        o.status !== 'PENDING' || o.mealCode || o.mealCode_2 || o.beverages.length > 0
    ).map(o => {
        // 建立簡化版物件 (Minified Object)
        // i: id, s: status, m1: meal1, m2: meal2, sp1: isSPML, sp2: isSPML_2
        // v1: mealServed, v2: mealServed_2
        const minOrder = { i: o.id };
        
        if (o.status !== 'PENDING') minOrder.s = o.status.substring(0, 1); // O=ORDERED, D=DELAY, N=DND
        if (o.status === 'DND') minOrder.s = 'X'; // Special char for DND
        ...
        if (o.appetizerChoice) minOrder.ap = o.appetizerChoice; // Appetizer
        
        return minOrder;
    });

    const payload = {
        f: flightNumber, // Flight No
...
function decompressAndMerge(jsonString) {
    try {
        ...
        renderSeatLayout();
        showMessage(`Sync Success! Updated ${updateCount} seats.`, false);
        closeSyncModal();
...
function switchTab(mode) {
    if (mode === 'send') {
        ...
        stopScanner();
        setTimeout(generateQR, 100); // Delay slightly to ensure UI is ready
    } else {
...
function generateQR() {
    ...
    // QR Code 上限約 2000-3000 字元，視乎複雜度
